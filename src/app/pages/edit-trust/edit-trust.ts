...
export class EditTrust implements OnInit, AfterViewInit {
  ...
  constructor() {
    ...
    this.trustForm
      .get('isBullionMember')!
      .valueChanges.subscribe((isMember: boolean) => {
    ...
    this.trustForm
      .get('wasReferredByMember')!
      .valueChanges.subscribe((wasReferred: boolean) => {
  ...
  private syncPropertyOwnerFromTrustee1(force = false): void {
    ...
    const isEmpty =
      !ownerCtrl.value || ownerCtrl.value?.toString().trim() === '';
  ...
  ngOnInit(): void {
    ...
    this.trustForm.patchValue(
      {
        signer_name:
          this.trustForm.get('signer_name')?.value || t2init.name || '',
        signer_id: this.trustForm.get('signer_id')?.value || t2init.id || '',
        signer_email:
          this.trustForm.get('signer_email')?.value || t2init.email || '',
      },
      { emitEvent: false }
    );
  ...
  ngAfterViewInit(): void {
    this.paymentMethodModalInstance = new bootstrap.Modal(
      this.paymentMethodModal.nativeElement,
      {
        backdrop: 'static',
        keyboard: false,
      }
    ...
    if (this.editTrustModal?.nativeElement) {
      this.editTrustModalInstance = new bootstrap.Modal(
        this.editTrustModal.nativeElement,
        {
          backdrop: 'static',
          keyboard: false,
        }
    ...
    queueMicrotask(() => {
      if (
        !this.isEditMode &&
        !this.editTrustNumber &&
        this.editTrustModalInstance
      ) {
        this.openEditModal();
      }
    });
    ...
    setTimeout(() => {
      ...
      if (fullName?.trim()) {
        if (
          !this.trustForm.get('isSettlor')?.value &&
          !this.settlor.get('name')?.value?.trim()
        ) {
          this.settlor.get('name')?.setValue(fullName, { emitEvent: false });
        }
        if (
          !this.trustForm.get('isTrustee')?.value &&
          !this.firstTrustee.get('name')?.value?.trim()
        ) {
          this.firstTrustee
            .get('name')
      ...
      if (idNumber?.trim()) {
        if (
          !this.trustForm.get('isSettlor')?.value &&
          !this.settlor.get('id')?.value?.trim()
        ) {
          this.settlor.get('id')?.setValue(idNumber, { emitEvent: false });
        }
        if (
          !this.trustForm.get('isTrustee')?.value &&
          !this.firstTrustee.get('id')?.value?.trim()
        ) {
  ...
  async updateXrpQuote(): Promise<void> {
    const apiUrl =
      'https://api.coingecko.com/api/v3/simple/price?ids=ripple&vs_currencies=usd,zar';
  ...
  copyToClipboard(text: string): void {
    if ((navigator as any)?.clipboard?.writeText) {
      navigator.clipboard
        .writeText(text)
        .then(() =>
          this.toastr?.success?.('Address copied', undefined, { timeOut: 2000 })
        )
        .catch(() =>
  ...
  checkBeneficiariesAgainstTrustees(): void {
    const beneficiaries =
      this.trustForm.get('beneficiaries')?.value?.toLowerCase() || '';
    ...
    const conflict = trusteeNames.find((trustee) =>
      beneficiaries.includes(trustee)
    );
    if (conflict) {
  ...
  onTrusteeCheckboxChange(event: Event): void {
    ...
    const settlorName =
      this.settlor.get('name')?.value ||
      this.trustForm.get('fullName')?.value ||
      '';
    const settlorId =
      this.settlor.get('id')?.value ||
      this.trustForm.get('idNumber')?.value ||
      '';
  ...
  onBeneficiariesBlur(): void {
    ...
    const isConflict = trusteeNames.some((name) =>
      beneficiaries.includes(name)
    );
    if (isConflict) {
      const modal = new bootstrap.Modal(
        this.trusteeConflictModal.nativeElement
      );
      modal.show();
    }
  }
  ...
  async confirmPaymentMethod(): Promise<void> {
    ...
    const trustDataForBackend = {
      ...rawForm,
      Property_Address: rawForm.propertyAddress || '',
    };
    ...
    if (selectedMethod === 'cardEFT') {
      ...
      try {
        ...
        const paymentInit = await this.http
          .post<any>(
        ...
        sessionStorage.setItem(
          'trustFormData',
          JSON.stringify(trustDataForBackend)
        );
  ...
  async handleCryptoManualConfirm(): Promise<void> {
    if (!this.isEditMode) {
      this.toastr.error(
        'Crypto payment for edits only on this page.',
        'Not in Edit Mode'
      );
      return;
    }
    if (!this.cryptoTxId || !this.isValidHex64(this.cryptoTxId)) {
      this.toastr.error(
        'Enter a valid 64-character hex transaction ID.',
        'Invalid TX ID'
      );
      return;
    }
  ...
  async performEditLookup(): Promise<void> {
    ...
    const trustNumber = (this.editLookupForm.value.trust_number || '').trim();
    const idOrPassport = (
      this.editLookupForm.value.id_or_passport || ''
    ).trim();

    const API_BASE = 'https://hongkongbackend.onrender.com';
    const url = `${API_BASE}/edit-trust/lookup?trust_number=${encodeURIComponent(
      trustNumber
    )}&id_or_passport=${encodeURIComponent(idOrPassport)}`;
    ...
    try {
      ...
      this.toastr.success(
        'Trust loaded. You can now edit and save changes.',
        'Edit mode enabled',
        {
          timeOut: 4000,
          closeButton: true,
      ...
      this.toastr.error(msg, 'Lookup Error', {
        timeOut: 6000,
        closeButton: true,
        progressBar: true,
      });
  ...
  private prefillFormFromLookup(rec: any): void {
    ...
    this.trustForm
      .get('isBullionMember')
      ?.setValue(!!rec.is_bullion_member, { emitEvent: true });
    this.trustForm
      .get('wasReferredByMember')
      ?.setValue(!!rec.referrer_number, { emitEvent: true });
    ...
    this.trustForm.patchValue(
      {
        ...
        establishmentDate:
          this.normalizeToISODate(rec.establishment_date) || '',
    ...
    this.firstTrustee.patchValue(
      { name: t1.name || '', id: t1.id || '' },
      { emitEvent: false }
    );
    this.secondTrustee.patchValue(
      { name: t2.name || '', id: t2.id || '' },
      { emitEvent: false }
    );
    this.thirdTrustee.patchValue(
      { name: t3.name || '', id: t3.id || '' },
      { emitEvent: false }
    );
    this.fourthTrustee.patchValue(
      { name: t4.name || '', id: t4.id || '' },
      { emitEvent: false }
    );
    ...
    this.settlor
      .get('email')
      ?.setValue(rec.settlor_email || '', { emitEvent: false });
    this.firstTrustee
      .get('email')
      ?.setValue(rec.trustee1_email || '', { emitEvent: false });
    this.secondTrustee
      .get('email')
      ?.setValue(rec.trustee2_email || '', { emitEvent: false });
    this.thirdTrustee
      .get('email')
      ?.setValue(rec.trustee3_email || '', { emitEvent: false });
    ...
    const fallbackSignerName =
      signerFromRecName || this.secondTrustee.get('name')?.value || '';
    const fallbackSignerId =
      signerFromRecId || this.secondTrustee.get('id')?.value || '';
    const fallbackSignerEmail =
      signerFromRecEmail || this.secondTrustee.get('email')?.value || '';
  ...
  private buildEditPayload(): any {
    ...
    const trustees = [
      ...
      (t) =>
        (t?.name && String(t.name).trim()) || (t?.id && String(t.id).trim())
    );
  ...
  async startEditPayment(): Promise<void> {
    if (!this.isEditMode || !this.editTrustNumber) {
      this.toastr.error(
        'Edit mode not active or trust number missing.',
        'Edit Payment Error',
        {
          timeOut: 5000,
          closeButton: true,
    ...
    const API_BASE = 'https://hongkongbackend.onrender.com';
    const url = `${API_BASE}/payments/edit-trust-payment-session/${encodeURIComponent(
      this.editTrustNumber
    )}`;
    try {
      ...
      const msg =
        err?.error?.detail || err?.message || 'Failed to start edit payment';
  ...
  async submitEdits(overridePayload?: any): Promise<void> {
    ...
    if (
      !overridePayload &&
      (this.firstTrustee.invalid || this.secondTrustee.invalid)
    ) {
      this.firstTrustee.markAllAsTouched();
      this.secondTrustee.markAllAsTouched();
      this.toastr.error(
        'Trust requires at least two trustees with name and ID.',
        'Validation',
        {
          timeOut: 5000,
          closeButton: true,
    ...
    const API_BASE = 'https://hongkongbackend.onrender.com';
    const url = `${API_BASE}/trusts/edit-trust/${encodeURIComponent(
      this.editTrustNumber
    )}`;
    ...
    try {
      ...
      this.toastr.success(
        'Amended deed generated and sent to admin.',
        'Trust updated',
        {
          timeOut: 6000,
          closeButton: true,
